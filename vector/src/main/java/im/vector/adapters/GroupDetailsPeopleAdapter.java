...
public class GroupDetailsPeopleAdapter extends AbsAdapter {
    ...
    private static final String LOG_TAG = GroupDetailsPeopleAdapter.class.getSimpleName();

    private static final int TYPE_JOINED_USERS = 12;

    private static final int TYPE_INVITED_USERS = 13;

    private final AdapterSection<GroupUser> mJoinedUsersSection;
    private final AdapterSection<GroupUser> mInvitedUsersSection;
    ...
    public GroupDetailsPeopleAdapter(final Context context, final OnSelectUserListener listener) {
        ...
        mJoinedUsersSection = new AdapterSection<>(context.getString(R.string.joined), -1,
                R.layout.adapter_item_contact_view, TYPE_HEADER_DEFAULT, TYPE_JOINED_USERS, new ArrayList<GroupUser>(), null);
        ...
        mInvitedUsersSection = new AdapterSection<>(context.getString(R.string.invited), -1,
                R.layout.adapter_item_contact_view, TYPE_HEADER_DEFAULT, TYPE_INVITED_USERS, new ArrayList<GroupUser>(), null);
    ...
    protected RecyclerView.ViewHolder createSubViewHolder(ViewGroup viewGroup, int viewType) {
        ...
        if ((viewType == TYPE_JOINED_USERS) || (viewType == TYPE_INVITED_USERS)) {
            return new GroupUserViewHolder(inflater.inflate(R.layout.adapter_item_contact_view, viewGroup, false));
        }
        return null;
    }
    ...
    int filterGroupUsersSection(final AdapterSection<GroupUser> section, final String filterPattern) {
        if (null != section) {
            if (!TextUtils.isEmpty(filterPattern)) {
                List<GroupUser> filteredGroupUsers = RoomUtils.getFilteredGroupUsers(section.getItems(), filterPattern);
