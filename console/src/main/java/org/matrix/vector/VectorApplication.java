...
import org.matrix.vector.contacts.ContactsManager;
import org.matrix.vector.contacts.PIDsRetriever;
import org.matrix.vector.ga.Analytics;
import org.matrix.vector.services.EventStreamService;
import org.matrix.vector.util.LogUtilities;
...
public class VectorApplication extends Application {
    ...
    public void startActivityTransitionTimer() {
        ...
        this.mActivityTransitionTimerTask = new TimerTask() {
            public void run() {
                VectorApplication.this.isInBackground = true;

                // suspend the events thread if the client uses GCM
                if (Matrix.getInstance(VectorApplication.this).getSharedGcmRegistrationManager().useGCM()) {
                    CommonActivityUtils.pauseEventStream(VectorApplication.this);
                }
        ...
        if (isInBackground) {
            ...
            if (Matrix.getInstance(VectorApplication.this).getSharedGcmRegistrationManager().useGCM()) {
                ...
                if (null == EventStreamService.getInstance()) {
                    CommonActivityUtils.startEventStreamService(VectorApplication.this);
                } else {
                    CommonActivityUtils.resumeEventStream(VectorApplication.this);
                }
            }
    ...
    public static boolean isAppInBackground() {
        if (mCurrentActivity != null) {
            return ((VectorApplication)(mCurrentActivity.getApplication())).isInBackground;
        }
        ...
        if (!TextUtils.isEmpty(trackerId)) {
            Analytics.initialiseGoogleAnalytics(this, trackerId, new ExceptionParser() {
                ...
                public String getDescription(String threadName, Throwable throwable) {
                    ...
                    Activity a = VectorApplication.getCurrentActivity();
                    if (a != null) {
                        b.append(", Activity:");
