...
class BackgroundSyncModeChooserDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let { activity: FragmentActivity ->
            ...
            view.findViewById<ListView>(R.id.dialog_background_sync_list)?.let {
                it.adapter = Adapter(
                        activity,
                        arguments!!.getString(ARG_INITIAL_MODE)
                )
            }
            builder.setView(view)
                    ...
                    .setPositiveButton(R.string.ok) { dialog, _ ->
                        ...
                        if (mode == arguments?.getString(ARG_INITIAL_MODE)) {
    ...
    data class SyncMode(val mode: String, val title: Int, val description: Int, var isSelected: Boolean)


    private class Adapter(context: Context, val initialMode: String) : ArrayAdapter<SyncMode>(context, 0, options) {
        init {
    ...
    interface InteractionListener {
        fun onCancel() {}
        fun onOptionSelected(mode: String) {}
    }

    companion object {
        private val options = listOf(
                SyncMode(PreferencesManager.FDROID_BACKGROUND_SYNC_MODE_FOR_BATTERY,
                ...
                SyncMode(PreferencesManager.FDROID_BACKGROUND_SYNC_MODE_FOR_REALTIME,
                ...
                SyncMode(PreferencesManager.FDROID_BACKGROUND_SYNC_MODE_DISABLED,
        ...
        fun newInstance(selectedMode: String, interactionListener: InteractionListener): BackgroundSyncModeChooserDialog {
            val frag = BackgroundSyncModeChooserDialog()
            frag.interactionListener = interactionListener
            val args = Bundle()
            args.putString(ARG_INITIAL_MODE, selectedMode)
            frag.arguments = args
            return frag
