...
class VectorSettingsPreferencesFragment : PreferenceFragment(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val gcmMgr = Matrix.getInstance(appContext)!!.sharedPushManager
        if (gcmMgr.usePush() && gcmMgr.hasRegistrationToken()) {
            // Hide the section
            ...
            backgroundSyncPreference.let {
                ...
                it.onPreferenceChangeListener = Preference.OnPreferenceChangeListener { preference, aNewValue ->
                    ...
                    Matrix.getInstance(activity)!!.sharedPushManager
                            .forceSessionsRegistration(object : PushManager.ThirdPartyRegistrationListener {
    ...
    private fun refreshDisplay() {
        ...
        val pushManager = Matrix.getInstance(appContext)!!.sharedPushManager
        ...
        mNotificationPrivacyPreference.isEnabled = !areNotificationAllowed && pushManager.areDeviceNotificationsAllowed() && pushManager.usePush()
    }
    ...
    private fun onPushRuleClick(fResourceText: String, newValue: Boolean) {
        val pushMgr = Matrix.getInstance(activity)!!.sharedPushManager
        ...
        if (fResourceText == PreferencesManager.SETTINGS_TURN_SCREEN_ON_PREFERENCE_KEY) {
            if (pushMgr.isScreenTurnedOn != newValue) {
                pushMgr.isScreenTurnedOn = newValue
            }
            return
        }
        ...
        if (fResourceText == PreferencesManager.SETTINGS_ENABLE_THIS_DEVICE_PREFERENCE_KEY) {
            val isConnected = Matrix.getInstance(activity)!!.isConnected
            val isAllowed = pushMgr.areDeviceNotificationsAllowed()
            ...
            if (isConnected && pushMgr.usePush() && (pushMgr.isServerRegistered || pushMgr.isServerUnRegistred)) {
                val listener = object : PushManager.ThirdPartyRegistrationListener {
    ...
    private fun refreshNotificationPrivacy() {
        val pushManager = Matrix.getInstance(activity)!!.sharedPushManager

        // this setting apply only with GCM for the moment
        if (pushManager.usePush()) {
    ...
    private fun refreshPushersList() {
        ...
        if (isNewList) {
            ...
            for (pusher in mDisplayedPushers) {
                if (null != pusher.lang) {
                    val isThisDeviceTarget = TextUtils.equals(pushManager.pushRegistrationToken, pusher.pushkey)
